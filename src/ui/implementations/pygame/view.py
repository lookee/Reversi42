...
"""
PygameBoardView - Pygame Implementation of Board Visualization

This is the primary graphical view using Pygame for rendering.
Implements the AbstractBoardView interface.
...
class PygameBoardView(AbstractBoardView):
    ...
    def __init__(
        self, sizex=64, sizey=48, width=480, heigth=400, theme: ColorPalette = Theme.PROFESSIONAL
    ):
    ...
    def calculateDimensions(self):
        ...
        self.marginx = (
            self.coord_margin + (self.width - self.coord_margin * 2 - board_pixel_width) // 2
        )
        self.marginy = (
            self.header_height
            + self.coord_margin
            + (self.heigth - self.header_height - self.coord_margin * 2 - board_pixel_height) // 2
        )
        ...
        if self.sizex == 8 and self.sizey == 8:
            ...
            for hoshi_x, hoshi_y in hoshi_positions:
                ...
                pygame.gfxdraw.filled_circle(
                    screen, intersection_x, intersection_y, hoshi_radius, self.hoshiColor
                )
    ...
    def unfillBox(self, bx, by):
        ...
        rect = Rect(
            self.marginx + bx * self.stepx, self.marginy + by * self.stepy, self.stepx, self.stepy
        )
        ...
        if self.sizex == 8 and self.sizey == 8:
            ...
            if (bx, by) in hoshi_positions:
                ...
                hoshi_radius = max(4, min(7, min(self.stepx, self.stepy) // 14))
                pygame.gfxdraw.filled_circle(
                    self.screen, intersection_x, intersection_y, hoshi_radius, self.hoshiColor
                )
                pygame.gfxdraw.aacircle(
                    self.screen, intersection_x, intersection_y, hoshi_radius, self.hoshiColor
                )
    ...
    def fillBox(self, bx, by, color, shadow=True, hollow=False):
        ...
        if hollow:
            ...
            if shadow:
                ...
                pygame.gfxdraw.filled_circle(
                    self.screen, shadow_x, shadow_y, radius, self.shadowColor
                )
    ...
    def drawLastMoveIndicator(self):
        ...
        if self.lastMoveX is not None and self.lastMoveY is not None:
            ...
            for i in range(3):
                ...
                pygame.gfxdraw.filled_circle(
                    self.screen, posx, posy, glow_radius, (255, 200, 80, 40 - i * 10)
                )
    ...
    def setCanMoveBlack(self, bx, by):
        ...
        for i in range(1, 4):
            ...
            if alpha > 0:
                pygame.draw.circle(
                    s, (*self.blackMoveColor, alpha), (center, center), radius + i, 1
                )
    ...
    def setCanMoveWhite(self, bx, by):
        ...
        for i in range(1, 4):
            ...
            if alpha > 0:
                pygame.draw.circle(
                    s, (*self.whiteMoveColor, alpha), (center, center), radius + i, 1
                )
    ...
    def drawHeader(self):
        ...
        if self.current_turn == "B":
            ...
            for i in range(3):
                ...
                alpha = 80 - i * 25
                pygame.gfxdraw.filled_circle(
                    self.screen, dot_x, dot_y, glow_radius, (*self.turn_indicator, alpha)
                )
            # Main dot
    ...
    def _draw_header_piece(self, x, y, radius, player):
        ...
        shadow_offset = 2
        pygame.gfxdraw.filled_circle(
            self.screen, x + shadow_offset, y + shadow_offset, radius, (0, 0, 0, 100)
        )
    ...
    # ========================================================================
    # AbstractBoardView Interface Implementation
    # ========================================================================

    def initialize(self):
        """Initialize Pygame screen and grid (implements AbstractBoardView)"""
        # Already called in __init__, but can be called again if needed
        pass

    def render_board(self, model):
        """Implements AbstractBoardView.render_board"""
        # Render all pieces from model
    ...
    def render_piece(self, x: int, y: int, color: str):
        """Implements AbstractBoardView.render_piece"""
        self.fillBox(x, y, color)

    def highlight_valid_moves(self, moves):
        """Implements AbstractBoardView.highlight_valid_moves"""
        for move in moves:
            # Moves can be (x, y) tuples or Move objects
            if hasattr(move, "x") and hasattr(move, "y"):
                # Move object (1-based)
                self.setCanMove(move.x - 1, move.y - 1)
            else:
                # Tuple (assuming 0-based)
                self.setCanMove(move[0], move[1])

    def highlight_last_move(self, x: int, y: int):
        """Implements AbstractBoardView.highlight_last_move"""
        # Assuming x, y are 1-based (from Move object)
        self.setLastMove(x - 1, y - 1)

    def update_display(self, cursor_mode: bool = False):
        """Implements AbstractBoardView.update_display"""
        self.update(cursor_mode)

    def clear_display(self):
        """Implements AbstractBoardView.clear_display"""
        self.screen.fill(self.bgColor)
        self.__init_grid()
    ...
    def point_to_board_position(self, screen_x: int, screen_y: int):
        """Implements AbstractBoardView.point_to_board_position"""
        return self.point2Box(screen_x, screen_y)
    ...
    def set_player_info(
        ...
        self.setPlayerNames(black_name, white_name)
        self.setPlayerCounts(black_count, white_count)
        self.setCurrentTurn(current_turn)

    def show_message(self, message: str, duration: float = 2.0):
        """Implements AbstractBoardView.show_message"""
        # Simple implementation - could be enhanced with proper message box
    ...
    def cleanup(self):
        """Implements AbstractBoardView.cleanup"""
        # Pygame cleanup handled by pygame.quit() in main
        pass
