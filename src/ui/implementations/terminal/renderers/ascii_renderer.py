...
from .ascii_theme import ASCIIColorScheme, ASCIITheme, ANSIColors
...
class ASCIIRenderer:
    ...
    def render_header(self, turn: str, black_count: int, white_count: int, 
                     move_count: int, black_name: str = "Black", 
    ...
    def render_board(self, board_state: List[List[str]], 
                    ...
                    book_moves: Optional[Dict[Tuple[int, int], int]] = None) -> str:
        """
    ...
    def _render_row(self, y: int, board_state: List[List[str]],
                   valid_moves: Optional[List[Tuple[int, int]]] = None,
                   last_move: Optional[Tuple[int, int]] = None,
                   book_moves: Optional[Dict[Tuple[int, int], int]] = None) -> str:
        """Render a single row"""
        ...
        for x in range(size):
            cell_content = self._render_cell(x, y, board_state, valid_moves, 
                                            last_move, book_moves)
            line += cell_content
            line += self.theme.board_grid + self.BOX_V + self.reset
        
        return line
    
    def _render_cell(self, x: int, y: int, board_state: List[List[str]],
                    valid_moves: Optional[List[Tuple[int, int]]] = None,
                    last_move: Optional[Tuple[int, int]] = None,
                    book_moves: Optional[Dict[Tuple[int, int], int]] = None) -> str:
        ...
        if piece == 'X':  # Black piece
            ...
            rank = next((i for i, ((mx, my), _) in enumerate(sorted_moves) 
                        if (mx, my) == (x, y)), 999)
