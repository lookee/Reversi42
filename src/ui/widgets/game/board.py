...
from ui.common import Theme, ColorPalette
...
class BoardWidget(Grid):
    ...
    def __init__(self, size: int = 8, cell_size: int = 50, 
                 theme: ColorPalette = Theme.PROFESSIONAL):
        """
    ...
    def _draw_grid(self, surface: pygame.Surface):
        ...
        for i in range(self.size + 1):
            ...
            pygame.draw.line(surface, self.theme.board_lines,
                           (self.rect.x, y),
                           (self.rect.x + self.rect.width, y), 2)
    ...
    def _draw_legal_moves(self, surface: pygame.Surface):
        ...
        for bx, by in self.legal_moves:
            ...
            pygame.draw.circle(surface, self.theme.legal_move, (center_x, center_y), 
                             self.cell_size // 6, 2)
    ...
    def _draw_book_moves(self, surface: pygame.Surface):
        ...
        for bx, by in self.book_moves:
            ...
            pygame.draw.rect(surface, self.theme.book_move,
        ...
        for by in range(self.size):
            for bx in range(self.size):
                ...
                if piece != '.':
                    ...
                    # Draw shadow
                    shadow_offset = 2
                    pygame.draw.circle(surface, self.theme.board_shadow,
                                     (center_x + shadow_offset, center_y + shadow_offset), radius)
    ...
    def _draw_last_move(self, surface: pygame.Surface):
        ...
        if self.last_move:
            ...
            center_x = self.rect.x + bx * self.cell_size + self.cell_size // 2
            center_y = self.rect.y + by * self.cell_size + self.cell_size // 2
            pygame.draw.circle(surface, self.theme.last_move, (center_x, center_y),
                             int(self.cell_size * 0.48), 3)
    ...
    def _draw_cursor(self, surface: pygame.Surface):
        ...
        if self.cursor_pos:
            ...
            pygame.draw.rect(surface, self.theme.cursor,
                           (x + 4, y + 4, self.cell_size - 8, self.cell_size - 8), 3)
