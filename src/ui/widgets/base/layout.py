...
class Row(HBox):
    ...
    def __init__(
        self, children: Optional[List[Widget]] = None, gap: int = 10, align: str = "top", **kwargs
    ):
        """
        Initialize row.
...
class Col(Container):
    ...
    def __init__(self, span: int = 12, child: Optional[Widget] = None, **kwargs):
        """
        Initialize column.
    ...
    def _center_child(self):
        ...
        y = (
            (self.rect.height - child.rect.height) // 2
            if self.rect.height > child.rect.height
            else 0
        )
        child.set_position(x, y)
