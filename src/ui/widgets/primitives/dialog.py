...
class InputDialog(Dialog):
    ...
    def __init__(self, title: str = "Input", prompt: str = "Enter text:",
                 default_text: str = "", width: int = 500, height: int = 200):
        """
        ...
        super().__init__(title=title, message="", buttons=["OK", "Cancel"],
    ...
    def _build_content(self, button_labels: List[str]):
        ...
        # Prompt
        prompt_label = Label(self.prompt, x=20, y=current_y, font_size=20, 
                            color=(200, 200, 210))
        self.add(prompt_label)
        current_y += 35
        
        # Input box
        self.input_box = InputBox(x=20, y=current_y, width=self.rect.width - 40, 
                                 height=40, default_text=self.default_text)
        self.add(self.input_box)
        current_y += 50
        
        # Instructions
        inst_label = Label("ENTER: OK | ESC: Cancel | Backspace: Delete", 
                          x=20, y=current_y, font_size=14, color=(150, 150, 160))
        self.add(inst_label)
...
class ListDialog(Dialog):
    ...
    def __init__(self, title: str = "Select", items: Optional[List[str]] = None,
                 allow_cancel: bool = True, width: int = 600, height: int = 500):
        """
        ...
        # Call parent with dummy message (will rebuild)
        super().__init__(title=title, message="", buttons=[],
                        width=width, height=height)
    ...
    def _build_content(self, button_labels: List[str]):
        ...
        for idx, item in enumerate(visible_items):
            ...
            btn = Button(display_item, x=20, y=current_y, 
                        width=self.rect.width - 40, height=item_height,
                        on_click=lambda i=idx: self._handle_item_click(i),
                        color=btn_color, text_color=text_color)
            self.add(btn)
        ...
        if len(display_items) > max_visible:
            scroll_label = Label(f"Showing {max_visible} of {len(display_items)} items", 
                                x=20, y=current_y, font_size=14, 
                                color=(150, 150, 160))
            self.add(scroll_label)
            current_y += 25
        
        # Instructions
        inst_label = Label("Arrows: Navigate | ENTER: Select | ESC: Cancel", 
                          x=20, y=self.rect.height - 50, font_size=14, 
                          color=(150, 150, 160))
        self.add(inst_label)
