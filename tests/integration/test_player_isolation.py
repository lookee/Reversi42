...
project_root = os.path.dirname(os.path.dirname(current_dir))  # Go up from tests/integration to project root
...
def test_player_creation():
    ...
    if hasattr(player1, 'bitboard_engine') and hasattr(player1.bitboard_engine, 'config'):
        ...
        if cfg1.search_strategy != expected_strategy:
            errors.append(f"   ❌ Strategy mismatch: expected {expected_strategy}, got {cfg1.search_strategy}")
        if cfg1.use_transposition_table != expected_tt:
            errors.append(f"   ❌ TT mismatch: expected {expected_tt}, got {cfg1.use_transposition_table}")
        if cfg1.use_parallel != expected_parallel:
            errors.append(f"   ❌ Parallel mismatch: expected {expected_parallel}, got {cfg1.use_parallel}")
        if cfg1.use_aspiration_windows != expected_aspiration:
            errors.append(f"   ❌ Aspiration mismatch: expected {expected_aspiration}, got {cfg1.use_aspiration_windows}")
    ...
    if hasattr(player2, 'bitboard_engine') and hasattr(player2.bitboard_engine, 'config'):
        ...
        if cfg2.search_strategy != expected_strategy:
            errors.append(f"   ❌ Strategy mismatch: expected {expected_strategy}, got {cfg2.search_strategy}")
        if cfg2.use_transposition_table != expected_tt:
            errors.append(f"   ❌ TT mismatch: expected {expected_tt}, got {cfg2.use_transposition_table}")
        if cfg2.use_parallel != expected_parallel:
            errors.append(f"   ❌ Parallel mismatch: expected {expected_parallel}, got {cfg2.use_parallel}")
        if cfg2.use_aspiration_windows != expected_aspiration:
            errors.append(f"   ❌ Aspiration mismatch: expected {expected_aspiration}, got {cfg2.use_aspiration_windows}")
