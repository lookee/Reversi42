...
def test_move_4_debug():
    ...
    for move_num in range(1, 5):
        ...
        print(f"\n🔍 INSTANCE CHECK:")
        print(f"   White instance: {current_white_id} (initial: {white_id_initial}) {'✅' if current_white_id == white_id_initial else '❌ CHANGED!'}")
        print(f"   Black instance: {current_black_id} (initial: {black_id_initial}) {'✅' if current_black_id == black_id_initial else '❌ CHANGED!'}")
        print(f"   White config: {current_white_cfg_id} (initial: {white_cfg_id_initial}) {'✅' if current_white_cfg_id == white_cfg_id_initial else '❌ CHANGED!'}")
        print(f"   Black config: {current_black_cfg_id} (initial: {black_cfg_id_initial}) {'✅' if current_black_cfg_id == black_cfg_id_initial else '❌ CHANGED!'}")
        ...
        print(f"\n🔍 CONFIGURATION CHECK:")
        print(f"   White depth: {white_cfg.depth} (expected: 4) {'✅' if white_cfg.depth == 4 else '❌ WRONG!'}")
        print(f"   White strategy: {white_cfg.search_strategy} (expected: fixed_depth) {'✅' if white_cfg.search_strategy == 'fixed_depth' else '❌ WRONG!'}")
        print(f"   Black depth: {black_cfg.depth} (expected: 12) {'✅' if black_cfg.depth == 12 else '❌ WRONG!'}")
        print(f"   Black strategy: {black_cfg.search_strategy} (expected: adaptive) {'✅' if black_cfg.search_strategy == 'adaptive' else '❌ WRONG!'}")
