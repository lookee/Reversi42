    ...
    def get_ai_move(self, side):
        ...
        if hasattr(ai, "bitboard_engine") and hasattr(ai.bitboard_engine, "config"):
            ...
            if ai_name == "LIGHTNING STRIKE":
                if cfg.depth != 4:
                    raise ValueError(
                        f"LIGHTNING STRIKE has wrong depth! Expected 4, got {cfg.depth}"
                    )
                if cfg.search_strategy != "fixed_depth":
                    raise ValueError(
                        f"LIGHTNING STRIKE has wrong strategy! Expected fixed_depth, got {cfg.search_strategy}"
                    )
                ...
                if cfg.search_strategy != "adaptive":
                    raise ValueError(
                        f"DIVZERO.EXE has wrong strategy! Expected adaptive, got {cfg.search_strategy}"
                    )
...
def test_webgui_simulation():
    ...
    while move_count < max_moves:
        ...
        if move_count == 4:
            ...
            print(
                f"      White strategy: {session.ai_white.bitboard_engine.config.search_strategy}"
            )
            print(
                f"      Black strategy: {session.ai_black.bitboard_engine.config.search_strategy}"
            )
