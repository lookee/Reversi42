...
class TestBoardStateIntegrity:
    ...
    def test_piece_count_after_move(self):
        ...
        assert new_count == initial_count + 1 + (initial_count - new_count + 1), \
...
class TestUndoFunctionality:
    ...
    def test_undo_multiple_moves(self):
        ...
        for i in range(moves_made):
            ...
            assert game.export_str() == expected_state, \
                f"Undo {i+1}: stato non corretto"
    ...
    def test_undo_redo_consistency(self):
        ...
        assert game.export_str() == state_after_moves, \
            "Undo/redo deve produrre lo stesso stato"
...
class TestBorderPositions:
    ...
    def test_corner_moves(self):
        ...
        for corner in corners:
            assert not game.valid_move(corner), \
    ...
    def test_all_board_positions(self):
        ...
        for y in range(1, 9):
            for x in range(1, 9):
                ...
                assert cell in [".", "B", "W"], \
                    f"Posizione [{y}][{x}] ha valore invalido: {cell}"
...
class TestInvalidMoves:
    ...
    def test_move_on_occupied_square(self):
        ...
        move = Move(4, 4)
        assert not game.valid_move(move), \
...
class TestStateConsistency:
    ...
    def test_move_stack_size(self):
        ...
        for i in range(5):
            ...
            if len(moves) == 0:
                break
            game.move(moves[0])
            assert len(game.board_position_stack) == i + 1, \
        ...
        for i in range(stack_size):
            ...
            assert len(game.board_position_stack) == expected_size, \
    ...
    def test_piece_count_invariant(self):
        ...
        for _ in range(10):
            ...
            # Verifica consistenza
            assert game.black_cnt == manual_black, \
                "black_cnt deve corrispondere al conteggio manuale"
            assert game.white_cnt == manual_white, \
        ...
        for _ in range(20):
            ...
            if len(moves) == 0:
                ...
                assert current_count == prev_count, \
                    "Pass turn non deve modificare il conteggio"
...
def test_complete_forward_backward_sequence():
    ...
    assert len(saved_states) == moves_played + 1, \
    ...
    for step in range(stack_size):
        ...
        assert after_pieces <= before_pieces, \
