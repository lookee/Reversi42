...
def assert_moves_match(game, position_name=""):
    ...
    assert len(moves_game) == len(moves_bitboard), \
    ...
    for move_str in moves_game:
        ...
        assert game.valid_move(move), \
            f"{position_name}: {move_str} should be valid in Game"
        assert bitboard.valid_move(move), \
...
class TestSpecificBoardConfigurations:
    ...
    def test_full_row(self):
        ...
        game.black_cnt = sum(1 for y in range(1, 9) for x in range(1, 9) if game.matrix[y][x] == 'B')
        game.white_cnt = sum(1 for y in range(1, 9) for x in range(1, 9) if game.matrix[y][x] == 'W')
        bitboard.black_cnt = bitboard._count_bits(bitboard.black)
        bitboard.white_cnt = bitboard._count_bits(bitboard.white)
...
def test_stress_test_1000_positions():
    ...
    for seed in range(50):  # 50 partite
        ...
        for _ in range(40):
            assert_moves_match(game, f"Stress test game {games_played+1}, position {positions_tested+1}")
            positions_tested += 1
    ...
    assert positions_tested >= 1000, \
        f"Should test 1000+ positions, tested {positions_tested}"
