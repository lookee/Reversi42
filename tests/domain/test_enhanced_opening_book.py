...
class TestEnhancedFeatures:
    ...
    def test_average_threshold(self):
        ...
        book_avg = get_enhanced_opening_book(
            score_threshold=0.0,
            use_average_threshold=True
        )
        
        book_fixed = get_enhanced_opening_book(
            score_threshold=0.0,
            use_average_threshold=False
        )
...
def test_not_used_by_existing_players():
    ...
    for player_file in player_files:
        with open(player_file, 'r') as f:
            ...
            assert 'EnhancedOpeningBook' not in content, \
                f"{player_file} uses EnhancedOpeningBook! Should use OpeningBook instead."
            
            assert 'get_enhanced_opening_book' not in content, \
