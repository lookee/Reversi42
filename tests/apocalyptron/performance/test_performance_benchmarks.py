...
class TestNodesPerSecond:
    ...
    def test_nps_midgame_vs_opening(self):
        ...
        nps_opening = stats_opening['search_stats']['nodes'] / elapsed_opening if elapsed_opening > 0 else 0
        ...
        nps_midgame = stats_midgame['search_stats']['nodes'] / elapsed_midgame if elapsed_midgame > 0 else 0
        
        # Entrambi dovrebbero avere NPS ragionevoli
        assert nps_opening > 500, f"Opening NPS too low: {nps_opening:.0f}"
        assert nps_midgame > 500, f"Midgame NPS too low: {nps_midgame:.0f}"
...
class TestTranspositionTablePerformance:
    ...
    def test_tt_hit_rate_increases_with_depth(self):
        ...
        if 3 in hit_rates and 5 in hit_rates:
            assert hit_rates[5] >= hit_rates[3] - 10, "TT hit rate should improve or stay similar with depth"
...
class TestPlayerApocalyptronPerformance:
    ...
    def test_player_depth_9_midgame(self):
        ...
        for m in [Move(6,5), Move(4,6), Move(3,5), Move(5,6), Move(6,6), Move(7,6), Move(5,5)]:
            if game.valid_move(m):
                game.move(m)
