    ...
    def test_console_observer_prints_output(self):
        ...
        try:
            ...
            observer.on_iteration_complete(
                depth=3, best_move=Move(3, 3), value=42, iteration_time=1.0
            )
            observer.on_search_complete(
                best_move=Move(3, 3), value=42, statistics={}, total_time=1.0
            )
...
class TestQuietObserver:
    ...
    def test_quiet_observer_produces_no_output(self):
        ...
        try:
            observer.on_search_start(depth=5, player_name="Test", game=BitboardGame())
            observer.on_iteration_complete(
                depth=1, best_move=Move(3, 3), value=0, iteration_time=1.0
            )
            observer.on_search_complete(
                best_move=Move(3, 3), value=0, statistics={}, total_time=1.0
            )
...
class TestObserverPattern:
    ...
    def test_multiple_observers(self):
        ...
        observers = [ConsoleObserver(), StatisticsObserver(), QuietObserver()]
    ...
    def test_observer_interface_compliance(self):
        ...
        required_methods = ["on_search_start", "on_depth_complete", "on_search_complete"]
        ...
        for observer in observers:
            for method in required_methods:
                ...
                if method == "on_depth_complete":
                    continue
                assert hasattr(observer, method), f"{observer.__class__.__name__} missing {method}"
    ...
    def test_statistics_observer_data_accuracy(self):
        ...
        if "best_move" in stats:
            assert (
                stats.get("best_move") == Move(5, 4) or stats.get("best_move") == "E4"
...
class TestObserverEdgeCases:
    ...
    def test_observer_handles_large_numbers(self):
        ...
        observer.on_iteration_complete(
            depth=12, best_move=Move(3, 3), value=100, iteration_time=1.0
        )
